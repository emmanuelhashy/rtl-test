...
describe("BookSearch", () => {
  ...
  beforeAll(async () => {
    // booksData = await readFile("./data/books.json");
  });
  const setup = () => render(<BookSearch books={booksData} />);
  ...
  describe("the correct elements were rendered", () => {
    ...
    it("should initially show all books", () => {
      setup();
      expect(screen.getAllByTestId("book")).toHaveLength(booksData.length);
    });

    it("should render data for each field in a book", () => {
      setup();
      const bookEls = screen.getAllByTestId("book");
      expect(bookEls).toHaveLength(booksData.length);
      const [book] = bookEls;
  ...
  describe("searching by each field alone", () => {
    describe("modifying the author field", () => {
      beforeEach(() => {
        setup();
        fireEvent.change(screen.getByTestId("author"), {target: {value: "woolf"}});
      });
      
      it("should respond to change events", () => {
        expect(screen.getByDisplayValue("woolf")).toBeInTheDocument();
      });
    ...
    describe("modifying the title field", () => {
      beforeEach(() => {
        setup();
        fireEvent.change(screen.getByTestId("title"), {target: {value: "dea"}});
      });

      it("should respond to change events", () => {
        expect(screen.getByDisplayValue("dea")).toBeInTheDocument();
      });
    ...
    describe("modifying the country field", () => {
      beforeEach(() => {
        setup();
        fireEvent.change(screen.getByTestId("country"), {target: {value: "ITALY"}});
      });
    ...
    describe("modifying the language field", () => {
      beforeEach(() => {
        setup();
        fireEvent.change(screen.getByTestId("language"), {target: {value: "SPanish"}});
      });
    ...
    describe("modifying the year field", () => {
      beforeEach(() => {
        setup();
        fireEvent.change(screen.getByTestId("year"), {target: {value: "196"}});
      });

      it("should respond to change events", () => {
        expect(screen.getByDisplayValue("196")).toBeInTheDocument();
      });

      it("should filter books on the year get", () => {
        const bookEls = screen.getAllByTestId("book");
